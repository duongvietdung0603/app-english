...
import { ArrowLeft, BookOpen, Clock, Target, Users, ChevronRight, Star, Award } from "lucide-react"
...
export default function TopicsPage() {
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        const response = await fetch(`/api/topics/${language}`)
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-lg font-semibold text-slate-100">Learning Topics</h1>
                  <p className="text-xs text-slate-400">
                    {languageFlags[language as keyof typeof languageFlags]}{" "}
                    {languageNames[language as keyof typeof languageNames]} Topics
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-emerald-300">
                  {topics.reduce((acc, topic) => acc + topic.vocabularyCount, 0)} Words
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-slate-100 mb-4">Choose Your Learning Topic</h2>
          <p className="text-lg text-slate-400 max-w-2xl mx-auto">
            Select a topic that interests you and start building your vocabulary with interactive lessons and practice
            exercises.
          </p>
        </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-slate-400">{topic.lessonsCount} lessons</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <Target className="h-3 w-3 text-slate-500" />
                    <span className="text-slate-400">{topic.vocabularyCount} words</span>
                  </div>
                  <div className="flex items-center space-x-1">
                ...
                <div className="space-y-1">
                  <div className="text-xs font-medium text-slate-400">What you'll learn:</div>
                ...
                {/* Action Button */}
                <Link href={`/learn/${language}/vocabulary?topic=${topic.id}`} className="block">
                  <Button className="w-full bg-blue-600 hover:bg-blue-700 group-hover:bg-blue-500 transition-all">
                    Start Learning
        ...
        {/* Achievement Section */}
        <div className="mt-12 bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/20 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-100">Complete Topics to Earn Badges</h3>
                <p className="text-slate-400">
                  Finish all lessons in a topic to unlock achievement badges and bonus points.
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
